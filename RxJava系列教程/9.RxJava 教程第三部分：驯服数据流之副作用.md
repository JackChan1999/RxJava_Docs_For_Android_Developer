...
前面两部分，我们学习到了如何创建 Observable 以及如何从 Observable 中获取数据。本部分将介绍一些更高级的用法，以及一些在大型项目中的最佳实践。
...
Functional programming 通常避免副作用。带有副作用的函数（尤其是可以修改参数状态的）要求开发者了解更多实现的细节。增加了函数的复杂度并且导致函数被错误理解和使用，并且难以维护。副作用有故意的和无意的。可以通过封装或者使用不可变对象来避免副作用。有一些明智的封装规则可以显著的提高你 Rx 代码的可维护性。
...
Rx 使用面向对象的 Java 语言来实现 functional programming 风格编码。 需要注意面向对象中的问题。 例如下面一个天真版的返回 observable 的服务：
